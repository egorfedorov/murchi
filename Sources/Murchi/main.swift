...
class CatRenderer {
    ...
    let size: CGFloat = 160  // Render at 2x for supersampling
    ...
    func getSprite(for behavior: PetBehavior, frame: Int, right: Bool) -> NSImage {
        ...
        case .lookingAtCursor:
            ...
            let quantized = Int(mousePos.x / 200)  // quantize to ~8 zones
            let cursorKey = "cat_cursor_\(quantized)_\(lookR)"
    ...
    func render(expression: CatExpression, pose: CatPose, lookRight: Bool) -> NSImage {
        ...
        let cx: CGFloat = 80
        let wobble: CGFloat = (pose == .walkL) ? -2 : (pose == .walkR ? 2 : 0)
        let headR: CGFloat = 46
        let headCX = cx + wobble
        let headCY: CGFloat = (pose == .lyingDown || pose == .lyingDown2) ? 84 : 90
        let bodyCY: CGFloat = (pose == .lyingDown || pose == .lyingDown2) ? 40 : 34
        let bodyW: CGFloat = 62
        let bodyH: CGFloat = (pose == .lyingDown || pose == .lyingDown2) ? 24 : 38

        // Duolingo palette — warm orange, NO outlines, gradient shading
        let main = NSColor(red: 1.0, green: 0.58, blue: 0.16, alpha: 1)
        let dark = NSColor(red: 0.90, green: 0.44, blue: 0.06, alpha: 1)
        let light = NSColor(red: 1.0, green: 0.72, blue: 0.38, alpha: 1)
        let belly = NSColor(red: 1.0, green: 0.92, blue: 0.80, alpha: 1)
        let pink = NSColor(red: 1.0, green: 0.50, blue: 0.48, alpha: 0.35)
        let headShadow = NSColor(red: 0.80, green: 0.34, blue: 0.04, alpha: 0.18)
        let eyeC = NSColor(red: 0.12, green: 0.10, blue: 0.16, alpha: 1)

        // ── TAIL ──
        if pose != .lyingDown && pose != .lyingDown2 {
            let tw: CGFloat = (pose == .walkL) ? -6 : (pose == .walkR ? 6 : 0)
            let tp = CGMutablePath()
            tp.move(to: CGPoint(x: headCX + 28, y: bodyCY + 2))
            tp.addQuadCurve(to: CGPoint(x: headCX + 34 + tw, y: bodyCY + 38), control: CGPoint(x: headCX + 46, y: bodyCY + 18))
            ctx.setLineCap(.round)
            ctx.setStrokeColor(dark.cgColor); ctx.setLineWidth(10)
            ctx.addPath(tp); ctx.strokePath()
            ctx.setStrokeColor(main.cgColor); ctx.setLineWidth(7)
            ctx.addPath(tp); ctx.strokePath()
        }

        // ── EARS — smooth curves, no outline ──
        for side in [-1.0, 1.0] as [CGFloat] {
            let ex = headCX + side * 30
            let ey = headCY + headR - 8
            let ear = CGMutablePath()
            ear.move(to: CGPoint(x: ex - 14, y: ey - 2))
            ear.addQuadCurve(to: CGPoint(x: ex, y: ey + 24), control: CGPoint(x: ex - 11, y: ey + 18))
            ear.addQuadCurve(to: CGPoint(x: ex + 14, y: ey - 2), control: CGPoint(x: ex + 11, y: ey + 18))
            ear.closeSubpath()
            ctx.setFillColor(dark.cgColor)
            ctx.addPath(ear); ctx.fillPath()
            // Inner ear — lighter
            let inner = CGMutablePath()
            inner.move(to: CGPoint(x: ex - 8, y: ey + 2))
            inner.addQuadCurve(to: CGPoint(x: ex, y: ey + 18), control: CGPoint(x: ex - 6, y: ey + 14))
            inner.addQuadCurve(to: CGPoint(x: ex + 8, y: ey + 2), control: CGPoint(x: ex + 6, y: ey + 14))
            inner.closeSubpath()
            ctx.setFillColor(main.cgColor)
            ctx.addPath(inner); ctx.fillPath()
            // Pink inner triangle
            let pnk = CGMutablePath()
            pnk.move(to: CGPoint(x: ex - 4, y: ey + 5))
            pnk.addLine(to: CGPoint(x: ex, y: ey + 14))
            pnk.addLine(to: CGPoint(x: ex + 4, y: ey + 5))
            pnk.closeSubpath()
            ctx.setFillColor(pink.cgColor)
            ctx.addPath(pnk); ctx.fillPath()
        }

        // ── BODY — large chunky oval, darker shade at bottom ──
        let bodyRect = CGRect(x: headCX - bodyW/2, y: bodyCY - bodyH/2, width: bodyW, height: bodyH)
        // Dark base
        ctx.setFillColor(dark.cgColor)
        ctx.addPath(CGPath(ellipseIn: bodyRect, transform: nil)); ctx.fillPath()
        // Main fill slightly higher
        let bodyMain = bodyRect.offsetBy(dx: 0, dy: 2).insetBy(dx: 1, dy: 1)
        ctx.setFillColor(main.cgColor)
        ctx.addPath(CGPath(ellipseIn: bodyMain, transform: nil)); ctx.fillPath()
        // Light upper area
        let bodyLt = CGRect(x: headCX - bodyW * 0.35, y: bodyCY, width: bodyW * 0.7, height: bodyH * 0.38)
        ctx.setFillColor(light.cgColor)
        ctx.addPath(CGPath(ellipseIn: bodyLt, transform: nil)); ctx.fillPath()
        // Belly
        let bR = CGRect(x: headCX - bodyW * 0.20, y: bodyCY - bodyH * 0.18, width: bodyW * 0.40, height: bodyH * 0.36)
        ctx.setFillColor(belly.cgColor)
        ctx.addPath(CGPath(ellipseIn: bR, transform: nil)); ctx.fillPath()

        // ── LEGS/PAWS — drawn after body so feet peek out at bottom ──
        if pose != .lyingDown && pose != .lyingDown2 {
            let pawW: CGFloat = 18; let pawH: CGFloat = 14
            let pawY = bodyCY - bodyH/2 - 6
            let lOff: CGFloat = (pose == .walkL) ? 4 : 0
            let rOff: CGFloat = (pose == .walkR) ? 4 : 0
            for (px, off) in [(headCX - 20, lOff), (headCX + 2, rOff)] {
                let pr = CGRect(x: px, y: pawY + off, width: pawW, height: pawH)
                ctx.setFillColor(dark.cgColor)
                ctx.addPath(CGPath(roundedRect: pr, cornerWidth: 7, cornerHeight: 7, transform: nil)); ctx.fillPath()
            }
        }

        // ── SHADOW from head onto body ──
        ctx.setFillColor(headShadow.cgColor)
        ctx.addEllipse(in: CGRect(x: headCX - 30, y: headCY - headR - 4, width: 60, height: 12)); ctx.fillPath()

        // ── HEAD — main circle with volume shading ──
        let headRect = CGRect(x: headCX - headR, y: headCY - headR, width: headR * 2, height: headR * 2)
        // Darker base for bottom shadow
        ctx.setFillColor(dark.cgColor)
        ctx.addPath(CGPath(ellipseIn: headRect.offsetBy(dx: 0, dy: -2), transform: nil)); ctx.fillPath()
        // Main head
        ctx.setFillColor(main.cgColor)
        ctx.addPath(CGPath(ellipseIn: headRect, transform: nil)); ctx.fillPath()
        // Light gradient upper area
        let headLt = CGRect(x: headCX - headR * 0.65, y: headCY + headR * 0.05, width: headR * 1.3, height: headR * 0.85)
        ctx.setFillColor(light.cgColor)
        ctx.addPath(CGPath(ellipseIn: headLt, transform: nil)); ctx.fillPath()
        // Bright spot upper-center
        ctx.setFillColor(NSColor(white: 1, alpha: 0.06).cgColor)
        ctx.addEllipse(in: CGRect(x: headCX - 16, y: headCY + 14, width: 32, height: 22)); ctx.fillPath()

        // ── FACE ──
        let faceCY = headCY - 8

        // Whiskers (cat-specific)
        ctx.setStrokeColor(dark.cgColor); ctx.setLineWidth(1.5)
        for (baseX, dir) in [(headCX - 22, CGFloat(-1)), (headCX + 22, CGFloat(1))] {
            ctx.move(to: CGPoint(x: baseX, y: faceCY - 6))
            ctx.addLine(to: CGPoint(x: baseX + dir * 18, y: faceCY - 2)); ctx.strokePath()
            ctx.move(to: CGPoint(x: baseX, y: faceCY - 10))
            ctx.addLine(to: CGPoint(x: baseX + dir * 18, y: faceCY - 10)); ctx.strokePath()
            ctx.move(to: CGPoint(x: baseX, y: faceCY - 14))
            ctx.addLine(to: CGPoint(x: baseX + dir * 16, y: faceCY - 18)); ctx.strokePath()
        }

        // Cheek spots (lighter, like reference)
        ctx.setFillColor(NSColor(red: 0.60, green: 0.88, blue: 0.20, alpha: 0).cgColor)
        // Use warm blush instead
        ctx.setFillColor(NSColor(red: 1.0, green: 0.45, blue: 0.38, alpha: expression == .love ? 0.25 : 0.10).cgColor)
        ctx.addEllipse(in: CGRect(x: headCX - 42, y: faceCY - 16, width: 14, height: 10)); ctx.fillPath()
        ctx.addEllipse(in: CGRect(x: headCX + 28, y: faceCY - 16, width: 14, height: 10)); ctx.fillPath()

        // ── EYES — WHITE SCLERA + dark iris + white highlight (Duolingo style) ──
        let eyeW: CGFloat = 26; let eyeH: CGFloat = 28
        let eyeGap: CGFloat = 4
        let eyeY = faceCY - 4
        let eLx = headCX - eyeGap/2 - eyeW
        let eRx = headCX + eyeGap/2

        func duoEye(_ x: CGFloat, _ y: CGFloat, _ w: CGFloat, _ h: CGFloat) {
            // White sclera
            ctx.setFillColor(NSColor.white.cgColor)
            ctx.addEllipse(in: CGRect(x: x, y: y, width: w, height: h)); ctx.fillPath()
            // Dark iris (large, fills most of eye)
            let irisW = w * 0.72; let irisH = h * 0.72
            let ix = x + (w - irisW) / 2
            let iy = y + (h - irisH) / 2 - 1
            ctx.setFillColor(eyeC.cgColor)
            ctx.addEllipse(in: CGRect(x: ix, y: iy, width: irisW, height: irisH)); ctx.fillPath()
            // Big white highlight (upper-left, ~35% of iris)
            let hl = irisW * 0.38
            ctx.setFillColor(NSColor.white.cgColor)
            ctx.addEllipse(in: CGRect(x: ix + 2, y: iy + irisH - hl - 2, width: hl, height: hl)); ctx.fillPath()
            // Small secondary highlight
            let sh = irisW * 0.16
            ctx.addEllipse(in: CGRect(x: ix + irisW - sh - 3, y: iy + 2, width: sh, height: sh)); ctx.fillPath()
        }

        switch expression {
        case .neutral, .curious:
            duoEye(eLx, eyeY, eyeW, eyeH)
            duoEye(eRx, eyeY, eyeW, eyeH)
        case .happy, .love:
            // Filled crescents on white base
            for ex in [eLx, eRx] {
                ctx.setFillColor(NSColor.white.cgColor)
                ctx.addEllipse(in: CGRect(x: ex, y: eyeY, width: eyeW, height: eyeH * 0.6)); ctx.fillPath()
                ctx.setFillColor(eyeC.cgColor)
                let arc = CGMutablePath()
                arc.move(to: CGPoint(x: ex + 1, y: eyeY + 6))
                arc.addQuadCurve(to: CGPoint(x: ex + eyeW - 1, y: eyeY + 6), control: CGPoint(x: ex + eyeW/2, y: eyeY + eyeH * 0.65))
                arc.addQuadCurve(to: CGPoint(x: ex + 1, y: eyeY + 6), control: CGPoint(x: ex + eyeW/2, y: eyeY + eyeH * 0.65 - 7))
                ctx.addPath(arc); ctx.fillPath()
            }
        case .excited:
            duoEye(eLx - 1, eyeY - 1, eyeW + 2, eyeH + 2)
            duoEye(eRx - 1, eyeY - 1, eyeW + 2, eyeH + 2)
        case .sleeping:
            ctx.setStrokeColor(eyeC.cgColor); ctx.setLineWidth(2.5)
            for ex in [eLx, eRx] {
                let p = CGMutablePath()
                p.move(to: CGPoint(x: ex + 3, y: eyeY + eyeH/2))
                p.addQuadCurve(to: CGPoint(x: ex + eyeW - 3, y: eyeY + eyeH/2), control: CGPoint(x: ex + eyeW/2, y: eyeY + eyeH/2 - 4))
        ...
        case .blink:
            ...
            ctx.setStrokeColor(eyeC.cgColor); ctx.setLineWidth(2.5)
            let p = CGMutablePath()
            p.move(to: CGPoint(x: eRx + 3, y: eyeY + eyeH/2))
            p.addQuadCurve(to: CGPoint(x: eRx + eyeW - 3, y: eyeY + eyeH/2), control: CGPoint(x: eRx + eyeW/2, y: eyeY + eyeH/2 - 4))
            ctx.addPath(p); ctx.strokePath()
        case .eating:
            for ex in [eLx, eRx] {
                ctx.setFillColor(NSColor.white.cgColor)
                ctx.addEllipse(in: CGRect(x: ex, y: eyeY, width: eyeW, height: eyeH * 0.55)); ctx.fillPath()
                ctx.setFillColor(eyeC.cgColor)
                let arc = CGMutablePath()
                arc.move(to: CGPoint(x: ex + 2, y: eyeY + 6))
                arc.addQuadCurve(to: CGPoint(x: ex + eyeW - 2, y: eyeY + 6), control: CGPoint(x: ex + eyeW/2, y: eyeY + eyeH * 0.55))
                arc.addQuadCurve(to: CGPoint(x: ex + 2, y: eyeY + 6), control: CGPoint(x: ex + eyeW/2, y: eyeY + eyeH * 0.55 - 6))
                ctx.addPath(arc); ctx.fillPath()
            }
        case .annoyed:
            let sq = eyeH - 10
            for ex in [eLx + 2, eRx + 2] {
                ctx.setFillColor(NSColor.white.cgColor)
                ctx.addEllipse(in: CGRect(x: ex, y: eyeY + 3, width: eyeW - 4, height: sq)); ctx.fillPath()
                ctx.setFillColor(eyeC.cgColor)
                ctx.addEllipse(in: CGRect(x: ex + 4, y: eyeY + 5, width: eyeW - 12, height: sq - 4)); ctx.fillPath()
                ctx.setFillColor(NSColor.white.cgColor)
                ctx.addEllipse(in: CGRect(x: ex + 5, y: eyeY + sq - 4, width: 5, height: 5)); ctx.fillPath()
            }
            ctx.setStrokeColor(eyeC.cgColor); ctx.setLineWidth(2.5)
            ctx.move(to: CGPoint(x: eLx, y: eyeY + eyeH + 1)); ctx.addLine(to: CGPoint(x: eLx + eyeW, y: eyeY + eyeH + 4)); ctx.strokePath()
            ctx.move(to: CGPoint(x: eRx + eyeW, y: eyeY + eyeH + 1)); ctx.addLine(to: CGPoint(x: eRx, y: eyeY + eyeH + 4)); ctx.strokePath()
        case .shocked:
            for ex in [eLx - 1, eRx - 1] {
                ctx.setFillColor(NSColor.white.cgColor)
                ctx.addEllipse(in: CGRect(x: ex, y: eyeY - 2, width: eyeW + 2, height: eyeH + 2)); ctx.fillPath()
                ctx.setFillColor(eyeC.cgColor)
                ctx.addEllipse(in: CGRect(x: ex + 7, y: eyeY + 5, width: 12, height: 12)); ctx.fillPath()
                ctx.setFillColor(NSColor.white.cgColor)
                ctx.addEllipse(in: CGRect(x: ex + 8, y: eyeY + 11, width: 4, height: 4)); ctx.fillPath()
            }
        case .sick:
            ctx.setStrokeColor(eyeC.cgColor); ctx.setLineWidth(2)
            for ex in [eLx + 5, eRx + 5] {
                let sp = CGMutablePath()
                sp.addArc(center: CGPoint(x: ex + 7, y: eyeY + 12), radius: 6, startAngle: 0, endAngle: .pi * 1.5, clockwise: false)
                ctx.addPath(sp); ctx.strokePath()
            }
        case .straining:
            ctx.setStrokeColor(eyeC.cgColor); ctx.setLineWidth(2.5)
            for ex in [eLx + 4, eRx + 4] {
                ctx.move(to: CGPoint(x: ex, y: eyeY + 4)); ctx.addLine(to: CGPoint(x: ex + 14, y: eyeY + 18)); ctx.strokePath()
                ctx.move(to: CGPoint(x: ex + 14, y: eyeY + 4)); ctx.addLine(to: CGPoint(x: ex, y: eyeY + 18)); ctx.strokePath()
            }
        }

        // ── NOSE — small yellow-orange triangle ──
        let noseY = faceCY - 12
        ctx.setFillColor(NSColor(red: 1.0, green: 0.75, blue: 0.15, alpha: 1).cgColor)
        let np = CGMutablePath()
        np.move(to: CGPoint(x: headCX - 4, y: noseY + 4))
        np.addQuadCurve(to: CGPoint(x: headCX + 4, y: noseY + 4), control: CGPoint(x: headCX, y: noseY))
        np.addQuadCurve(to: CGPoint(x: headCX, y: noseY + 7), control: CGPoint(x: headCX + 2, y: noseY + 6))
        np.addQuadCurve(to: CGPoint(x: headCX - 4, y: noseY + 4), control: CGPoint(x: headCX - 2, y: noseY + 6))
        ctx.addPath(np); ctx.fillPath()

        // ── MOUTH — small cat omega ──
        ctx.setStrokeColor(NSColor(red: 0.30, green: 0.18, blue: 0.10, alpha: 0.55).cgColor); ctx.setLineWidth(1.8)
        let mY = noseY - 2
        switch expression {
        case .happy, .love, .excited:
            let mp = CGMutablePath()
            mp.move(to: CGPoint(x: headCX - 7, y: mY))
            mp.addQuadCurve(to: CGPoint(x: headCX, y: mY - 2), control: CGPoint(x: headCX - 3, y: mY - 5))
            mp.addQuadCurve(to: CGPoint(x: headCX + 7, y: mY), control: CGPoint(x: headCX + 3, y: mY - 5))
            ctx.addPath(mp); ctx.strokePath()
        case .eating:
            ctx.setFillColor(NSColor(red: 0.30, green: 0.10, blue: 0.06, alpha: 0.6).cgColor)
            ctx.addEllipse(in: CGRect(x: headCX - 4, y: mY - 6, width: 8, height: 6)); ctx.fillPath()
        case .shocked:
            ctx.setFillColor(NSColor(red: 0.30, green: 0.10, blue: 0.06, alpha: 0.7).cgColor)
            ctx.addEllipse(in: CGRect(x: headCX - 5, y: mY - 8, width: 10, height: 8)); ctx.fillPath()
        default:
            let mp = CGMutablePath()
            mp.move(to: CGPoint(x: headCX - 5, y: mY))
            mp.addQuadCurve(to: CGPoint(x: headCX, y: mY - 2), control: CGPoint(x: headCX - 2, y: mY - 3))
            mp.addQuadCurve(to: CGPoint(x: headCX + 5, y: mY), control: CGPoint(x: headCX + 2, y: mY - 3))
            ctx.addPath(mp); ctx.strokePath()
        }
        ...
        if expression == .sleeping {
            let attrs: [NSAttributedString.Key: Any] = [
                ...
                .foregroundColor: dark.withAlphaComponent(0.4)
            ]
            ctx.textPosition = CGPoint(x: headCX + headR - 2, y: headCY + 6)
            CTLineDraw(CTLineCreateWithAttributedString(NSAttributedString(string: "z", attributes: attrs)), ctx)
            ctx.textPosition = CGPoint(x: headCX + headR + 6, y: headCY + 18)
            CTLineDraw(CTLineCreateWithAttributedString(NSAttributedString(string: "Z", attributes: attrs)), ctx)
        }
...
struct Sprites {
    ...
    static let walk1: [[UInt32]] = {
        ...
        let G: UInt32 = 0xFF9433, GD: UInt32 = 0xE66B1A, W: UInt32 = 0xE8E8F0
    ...
    static let sleep1: [[UInt32]] = {
        let T: UInt32 = 0, B: UInt32 = 0x2D2D3F
        let G: UInt32 = 0xFF9433, P: UInt32 = 0xFFAABB, Z: UInt32 = 0x88AAFF
        return [
    ...
    static let sad: [[UInt32]] = {
        let T: UInt32 = 0, B: UInt32 = 0x2D2D3F
        let G: UInt32 = 0xFF9433, W: UInt32 = 0xFFF5EB
        let P: UInt32 = 0xFFAABB, GR: UInt32 = 0x7BCE7B
    ...
    static let sick: [[UInt32]] = {
        let T: UInt32 = 0, B: UInt32 = 0x2D2D3F
        let G: UInt32 = 0x9ABF70, GD: UInt32 = 0x7A9F50, W: UInt32 = 0xFFF5EB  // sick greenish body
    ...
    static let pawPrint: [[UInt32]] = {
        let T: UInt32 = 0, P: UInt32 = 0xE66B1A
        return [
            [T,P,T,P,T],
            [P,T,T,T,P],
            [T,P,P,P,T],
...
class ParticleSystem {
    ...
    func emit(at point: NSPoint, type: Particle.ParticleType, count: Int = 5) {
        ...
        let actualCount = min(count, 300 - particles.count)
...
class BearRenderer {
    ...
    func getSprite(for behavior: PetBehavior, frame: Int, right: Bool) -> NSImage {
        ...
        case .lookingAtCursor:
            ...
            let cursorKey = "bear_cursor_\(quantized)_\(lookR)"
            if let cached = cache[cursorKey] { return cached }
            let img = render(expression: .curious, pose: .standing, lookRight: lookR)
            cacheInsert(cursorKey, img)
            return img
    ...
    func render(expression: BearExpression, pose: BearPose, lookRight: Bool) -> NSImage {
        ...
        let headCY: CGFloat = (pose == .lyingDown || pose == .lyingDown2) ? 84 : 90
        let bodyCY: CGFloat = (pose == .lyingDown || pose == .lyingDown2) ? 40 : 34
        let bodyW: CGFloat = 64
        let bodyH: CGFloat = (pose == .lyingDown || pose == .lyingDown2) ? 24 : 38

        // White bear palette — soft whites and grays, NO outlines
        let main = NSColor(red: 0.95, green: 0.95, blue: 0.96, alpha: 1)
        let dark = NSColor(red: 0.82, green: 0.82, blue: 0.85, alpha: 1)
        let light = NSColor(red: 1.0, green: 1.0, blue: 1.0, alpha: 1)
        let belly = NSColor(red: 1.0, green: 0.98, blue: 0.94, alpha: 1)
        let pink = NSColor(red: 1.0, green: 0.55, blue: 0.55, alpha: 0.30)
        let headShadow = NSColor(red: 0.65, green: 0.65, blue: 0.70, alpha: 0.15)
        let eyeC = NSColor(red: 0.12, green: 0.10, blue: 0.16, alpha: 1)

        // ── EARS — round circles behind head ──
        let earR: CGFloat = 16
        for side in [-1.0, 1.0] as [CGFloat] {
            let ex = headCX + side * 36
            let ey = headCY + headR - 4
            // Darker outer
            ctx.setFillColor(dark.cgColor)
            ctx.addEllipse(in: CGRect(x: ex - earR, y: ey - earR, width: earR * 2, height: earR * 2)); ctx.fillPath()
            // Lighter inner
            ctx.setFillColor(main.cgColor)
            ctx.addEllipse(in: CGRect(x: ex - earR + 3, y: ey - earR + 3, width: earR * 2 - 6, height: earR * 2 - 6)); ctx.fillPath()
            // Pink inside
            ctx.setFillColor(pink.cgColor)
            ctx.addEllipse(in: CGRect(x: ex - 7, y: ey - 7, width: 14, height: 14)); ctx.fillPath()
        }

        // ── BODY — large chunky oval ──
        let bodyRect = CGRect(x: headCX - bodyW/2, y: bodyCY - bodyH/2, width: bodyW, height: bodyH)
        ctx.setFillColor(dark.cgColor)
        ctx.addPath(CGPath(ellipseIn: bodyRect, transform: nil)); ctx.fillPath()
        let bodyMain = bodyRect.offsetBy(dx: 0, dy: 2).insetBy(dx: 1, dy: 1)
        ctx.setFillColor(main.cgColor)
        ctx.addPath(CGPath(ellipseIn: bodyMain, transform: nil)); ctx.fillPath()
        let bodyLt = CGRect(x: headCX - bodyW * 0.35, y: bodyCY, width: bodyW * 0.7, height: bodyH * 0.36)
        ctx.setFillColor(light.cgColor)
        ctx.addPath(CGPath(ellipseIn: bodyLt, transform: nil)); ctx.fillPath()
        let bR = CGRect(x: headCX - bodyW * 0.22, y: bodyCY - bodyH * 0.20, width: bodyW * 0.44, height: bodyH * 0.40)
        ctx.setFillColor(belly.cgColor)
        ctx.addPath(CGPath(ellipseIn: bR, transform: nil)); ctx.fillPath()

        // ── LEGS/PAWS — drawn after body so feet peek out ──
        if pose != .lyingDown && pose != .lyingDown2 {
            let pawW: CGFloat = 20; let pawH: CGFloat = 14
            let pawY = bodyCY - bodyH/2 - 6
            let lOff: CGFloat = (pose == .walkL) ? 4 : 0
            let rOff: CGFloat = (pose == .walkR) ? 4 : 0
            for (px, off) in [(headCX - 22, lOff), (headCX + 2, rOff)] {
                let pr = CGRect(x: px, y: pawY + off, width: pawW, height: pawH)
                ctx.setFillColor(dark.cgColor)
                ctx.addPath(CGPath(roundedRect: pr, cornerWidth: 7, cornerHeight: 7, transform: nil)); ctx.fillPath()
            }
        ...
        ctx.setFillColor(main.cgColor)
        ctx.addPath(CGPath(ellipseIn: headRect, transform: nil)); ctx.fillPath()
        let headLt = CGRect(x: headCX - headR * 0.6, y: headCY + headR * 0.05, width: headR * 1.2, height: headR * 0.8)
        ctx.setFillColor(light.cgColor)
        ctx.addPath(CGPath(ellipseIn: headLt, transform: nil)); ctx.fillPath()
        ...
        // Cheeks
        ctx.setFillColor(NSColor(red: 1.0, green: 0.50, blue: 0.50, alpha: expression == .love ? 0.25 : 0.10).cgColor)
        ctx.addEllipse(in: CGRect(x: headCX - 44, y: faceCY - 18, width: 14, height: 10)); ctx.fillPath()
        ctx.addEllipse(in: CGRect(x: headCX + 30, y: faceCY - 18, width: 14, height: 10)); ctx.fillPath()

        // ── EYES — WHITE SCLERA + dark iris (Duolingo style) ──
        let eyeW: CGFloat = 26; let eyeH: CGFloat = 28
        let eyeGap: CGFloat = 4
        let eyeY = faceCY - 4
        let eLx = headCX - eyeGap/2 - eyeW
        ...
        // ── NOSE — dark oval with shine ──
        let noseY = faceCY - 13
        ctx.setFillColor(NSColor(red: 0.16, green: 0.14, blue: 0.20, alpha: 1).cgColor)
        ctx.addEllipse(in: CGRect(x: headCX - 6, y: noseY, width: 12, height: 9)); ctx.fillPath()
        ctx.setFillColor(NSColor(white: 1, alpha: 0.30).cgColor)
        ctx.addEllipse(in: CGRect(x: headCX - 2, y: noseY + 5, width: 4, height: 2)); ctx.fillPath()

        // ── MOUTH ──
        ctx.setStrokeColor(NSColor(red: 0.35, green: 0.30, blue: 0.35, alpha: 0.5).cgColor); ctx.setLineWidth(1.5)
        let mY = noseY - 2
        switch expression {
        case .happy, .love, .excited:
            let mp = CGMutablePath()
            mp.move(to: CGPoint(x: headCX - 6, y: mY))
            mp.addQuadCurve(to: CGPoint(x: headCX, y: mY - 2), control: CGPoint(x: headCX - 3, y: mY - 4))
            mp.addQuadCurve(to: CGPoint(x: headCX + 6, y: mY), control: CGPoint(x: headCX + 3, y: mY - 4))
            ctx.addPath(mp); ctx.strokePath()
        case .eating:
            ctx.setFillColor(NSColor(red: 0.25, green: 0.10, blue: 0.08, alpha: 0.5).cgColor)
            ctx.addEllipse(in: CGRect(x: headCX - 4, y: mY - 6, width: 8, height: 6)); ctx.fillPath()
        case .shocked:
            ctx.setFillColor(NSColor(red: 0.25, green: 0.10, blue: 0.08, alpha: 0.6).cgColor)
            ctx.addEllipse(in: CGRect(x: headCX - 4, y: mY - 8, width: 8, height: 7)); ctx.fillPath()
        default:
            let mp = CGMutablePath()
            mp.move(to: CGPoint(x: headCX - 4, y: mY))
            mp.addQuadCurve(to: CGPoint(x: headCX, y: mY - 1), control: CGPoint(x: headCX - 2, y: mY - 2))
            mp.addQuadCurve(to: CGPoint(x: headCX + 4, y: mY), control: CGPoint(x: headCX + 2, y: mY - 2))
            ctx.addPath(mp); ctx.strokePath()
        }
        ...
        if expression == .sleeping {
            let attrs: [NSAttributedString.Key: Any] = [
                .font: NSFont.systemFont(ofSize: 14, weight: .semibold),
                .foregroundColor: dark.withAlphaComponent(0.35)
            ]
            ctx.textPosition = CGPoint(x: headCX + headR - 2, y: headCY + 6)
            CTLineDraw(CTLineCreateWithAttributedString(NSAttributedString(string: "z", attributes: attrs)), ctx)
            ctx.textPosition = CGPoint(x: headCX + headR + 6, y: headCY + 18)
...
class MurchiDelegate: NSObject, NSApplicationDelegate {
    ...
    func setupPetWindow() {
        ...
        // Try Lottie rendering first, fall back to CGContext
        let charName = petType  // "cat", "bear", or "bird"
        if let manager = LottieCharacterManager(characterName: charName,
                                                 frame: NSRect(x: 0, y: 0, width: petSize, height: petSize)) {
            lottieManager = manager
            container.addSubview(manager.animationView)
            print("[Murchi] Using Lottie renderer for \(charName) (GPU-accelerated)")
        } else {
            // CGContext fallback — no .json Lottie file found
            lottieManager = nil
            ...
            print("[Murchi] Using CGContext renderer for \(charName) (no .json file found)")
        }
    ...
    func spawnPoopWindow(at x: CGFloat) {
        // Auto-clean oldest poops to prevent memory leak (max 8 on screen)
        while poopWindows.count >= 8 {
            poopWindows[0].orderOut(nil)
            poopWindows.removeFirst()
        }
    ...
    func startTimers() {
        ...
        mainUpdateTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 30.0, repeats: true) { [weak self] _ in
            self?.update()
        }

        // Stats decay — every 30 seconds
        statsDecayTimer = Timer.scheduledTimer(withTimeInterval: 30, repeats: true) { [weak self] _ in
            guard let self = self else { return }
            let now = Date()
            ...
            if self.stats.level > self.lastLevel {
                ...
                // Lottie doesn't have separate triggers — animation handled by update()
    ...
    func pickRandomBehavior() {
        ...
        case .happy:
            options = [
                ...
                (.zoomies, 5), (.scratching, 4), (.promenade, 5),
            ]
        case .neutral:
            options = [
    ...
    func update() {
        ...
        let needsPhysics = !isDragging && behavior != .sleeping && behavior != .edgeWalking
    ...
    func updateSprite() {
        if useLottie {
            // Lottie handles its own rendering via Core Animation
            // We just update the behavior → animation segment mapping
            let currentBehavior = isGentleDropping ? PetBehavior.jumping : behavior
            lottieManager?.update(behavior: currentBehavior, frame: animFrame, facingRight: facingRight)
            return
        }
        // CGContext fallback
        if isGentleDropping {
    ...
    func spawnPawPrint(at point: NSPoint) {
        ...
        let timer = Timer.scheduledTimer(withTimeInterval: 3.0, repeats: false) { [weak self] t in
            win.orderOut(nil)
            self?.pawPrintWindows.removeAll { $0 === win }
            self?.pawPrintTimers.removeAll { $0 === t }
        }
    ...
    @objc func feedPet() {
        ...
        // Lottie eat animation handled by behavior update
        particleCanvas.particleSystem.emit(
            at: NSPoint(x: petSize / 2 + 40, y: petSize + 10),
    ...
    @objc func selectPetType(_ sender: NSMenuItem) {
        ...
        // Switch Lottie character, fall back to CGContext
        switchLottieCharacter(to: petType)
        // Re-render current sprite
        updateSprite()
    }

    /// Switch Lottie character or fall back to CGContext when changing pet type
    func switchLottieCharacter(to newType: String) {
        guard let container = petWindow.contentView else { return }
        // Remove old Lottie view if present
        lottieManager?.animationView.removeFromSuperview()
        lottieManager = nil

        if let manager = LottieCharacterManager(characterName: newType,
                                                 frame: NSRect(x: 0, y: 0, width: petSize, height: petSize)) {
            lottieManager = manager
            container.addSubview(manager.animationView, positioned: .below, relativeTo: nil)
            petImageView?.isHidden = true
            print("[Murchi] Switched to Lottie renderer for \(newType)")
        } else {
            // CGContext fallback
            lottieManager = nil
            petImageView?.isHidden = false
            print("[Murchi] Using CGContext fallback for \(newType)")
        }
    }
    ...
    func petTapped() {
        ...
        if clickCount >= 3 {
            ...
            // Lottie pet animation handled by behavior update
        }
        stats.save()
